"""Root-level pytest fixtures shared across all test trees.

Pytest only inherits ``conftest.py`` files from ancestor directories, so
test suites under ``packages/*/tests/`` cannot see fixtures declared in
``tests/conftest.py``. Declaring them at the repo root makes them
